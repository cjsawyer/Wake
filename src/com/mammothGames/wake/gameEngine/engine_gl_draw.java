...
//                                ref.g_textures.texture_locations_arrays[sheetID-1]  [  (( subTextureID ) * 8  ) - 4  + 2  ]
         ...
         * @return
         */
        public float drawText(float x, float y, float size, int x_align, int y_align, int depth, char[] charArray, int charLength, int texture_sheet){
                if (ref.textureLoader.has_loaded[texture_sheet-1]) {
...
//                return ref.draw.drawText(x, y, size, ref.draw.X_ALIGN_CENTER, ref.draw.Y_ALIGN_TOP, 0, 200,  ref.strings.stringChars, ref.strings.builder.length(), ref.g_textures.TEX_FONT1);;
                return ref.draw.drawText(x, y, size, x_align, y_align, depth, ref.strings.stringChars, ref.strings.builder.length(), texture_sheet); 
        }
